<div class="relative" x-data="{
    ...
    <!-- User Button -->
<button
        class="flex items-center text-white dark:text-white"
        @click.prevent="toggleDropdown()"
        type="button"
    >
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11 flex items-center justify-center bg-white/10 border border-white/20">
...
<span class="block mr-1 font-medium text-theme-sm text-white">{{ Auth::user()->name }}</span>
    ...
    >
        <!-- User Info -->
        <div>
<span class="block font-medium text-black text-theme-sm dark:text-black">{{ Auth::user()->name }}</span>
<span class="mt-0.5 block text-theme-xs text-black/80 dark:text-black/80">{{ Auth::user()->email }}</span>
        </div>
        ...
        <form method="POST" action="/logout" class="w-full mt-3" @click="closeDropdown()">
            @csrf
